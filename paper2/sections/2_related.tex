...
average of its alters' outcomes, and Leenders~\cite{leenders2002modeling} makes the
...
Borgatti~\cite{everett2026alter} give the general matrix formulation of \emph{alter
...
Roeling and Nicholls~\cite{roeling2020imputation} impute unobserved attributes from
network structure and find the procedure sensitive to model misspecification and to
missingness. V\"or\"os et al.~\cite{voros2019limits} report a negative result of the
...
developed literature here: Bojanowski and Corten~\cite{bojanowski2014measuring}
survey and axiomatise segregation and homophily indices, and
McMillan~\cite{mcmillan2022worth} shows that homophily measured on strong and weak
ties differs in magnitude, so that a single homophily figure for a population can be
misleading. Noel and Nyhan~\cite{noel2011unfriending} demonstrate the consequence for
...
observation of this journal~\cite{lazega1999multiplexity}, and its methodological
...
V\"or\"os and Snijders~\cite{voros2017cluster}. Snijders et
al.~\cite{snijders2013model} show the stronger version: structure apparently present
in an advice relation is substantially mediated by the friendship relation beneath
it. An et al.~\cite{an2026estimating} find the same phenomenon quantitatively, with
...
Section~\ref{sec:measuring} finds camouflage differing by an order of magnitude
...
perceived, is established here but thinly populated. Burt et al.~\cite{burt2021network}
...
al.~\cite{shea2019cheater} come closest to our mechanism: actors anticipating
...
Coscia~\cite{barone2018birds} estimate trustworthiness from mutually inconsistent
...
misinformation susceptibility in personal networks~\cite{choi2026who}. We therefore
...
long-standing concern here. Snijders~\cite{snijders1981degree} introduced degree
variance as an index of graph heterogeneity; Anderson et al.~\cite{anderson1999interaction}
showed that graph-level indices are systematically confounded by size and density; and
Krivitsky et al.~\cite{krivitsky2009representing} exhibit networks with identical,
...
finds exactly this: our identity holds to within $3\%$ where class mean degrees are
comparable and requires correction where they are not.

The stochastic blockmodel we build on originates in this
journal~\cite{holland1983stochastic}. Degree correction \emph{within} blockmodels is
much less developed here; the one instance we found, Yu et
al.~\cite{yu2018detecting}, monitors node propensity in a dynamic degree-corrected
