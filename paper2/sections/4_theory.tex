...
better on the event $\{K\geq2\}$, and ties when $K\in\{0,1\}$. The fraction of nodes
that strictly benefit is
\begin{equation}
    \Pr[K \geq 2] \;=\; 1 - (1-\rho)^{\!D}\!\cdot\!\bigl(\ldots\bigr)
    \;=\; 1 - \rho^{D} - D(1-\rho)\rho^{D-1},
    \label{eq:benefit}
\end{equation}
...
$\mathbb{E}[K] = 1.55$ but $1/\mathbb{E}[1/K \mid K\geq1] = 1.29$, and $36\%$ of
...
a useful summary is the point at which half of them still benefit, which
is $\rho = 0.897$ at $D=16$ and $0.974$ at $D=64$.
\end{remark}
...
camouflage differs by an order of magnitude across relation types within one
